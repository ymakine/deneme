...
exports.run = async (bot, message, args) => {
    ...
    if (rMember.roles.has(aRole.id)) return message.channel.sendEmbed(new Discord.RichEmbed().setDescription('Böyle Bir Rolü Yok ki Zaten').setColor(10038562));
    await (rMember.addRole(aRole.id))
    message.channel.sendMessage(new Discord.RichEmbed().setDescription(`${rMember} isimli üyeye \`${role.name}\` isimli yetki başarıyla verildi! :white_check_mark:`).setColor('RANDOM'));
